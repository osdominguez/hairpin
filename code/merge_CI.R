...
for (as in c("as", "noas")) {
  ...
  write.table(boot_df,  file = paste0(our_dir,pheno_name, "_", as, "_bootstrap.table"), row.names = F, quote = F)  
  ...
  write.table(cofint_table, file = paste0(out_dir, phen_name, "_", as, "_confint.table"), row.names = F, quote = F)
}