...
if (length(args) != 5) {
  stop("Only five argument may be supplied", call.=FALSE)
} 
...
bootstrap_hairpin <- function(boot_num, pheno_filename, pheno_name, pheno_id, as) {
  ...
  hairpin_output <- make_hairpin(pheno_filename, pheno_name, pheno_id, boot_num) %>% mutate(replicate = boot_num)
  
  write.table(hairpin_output,  file = paste0(out_dir, "temp/", pheno_name, "_", as, "_bootstrap_", boot_num, ".table"), row.names = F, quote = F)
}
...
if (boot) {
  bootstrap_hairpin(run_n, phen_path, phen_name, phen_id)
} else {
  baseas <- make_hairpin(phen_path, phen_name, phen_id, "as")
  write.table(baseas, file = paste0(out_dir, phen_name, "_as_base.table"), row.names = F, quote = F)
  basenoas <- make_hairpin(phen_path, phen_name, phen_id, "noas")
  write.table(basenoas, file = paste0(out_dir, phen_name, "_noas_base.table"), row.names = F, quote = F)
}
